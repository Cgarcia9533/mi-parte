...
public class MainActivity extends Activity {
    ...
    /** OneDrive nunca cambio su id de paquete al dejar de llamarse SkyDrive. */
    private static final String PAQUETE_ONEDRIVE = "com.microsoft.skydrive";
    ...
    public final class Puente {
        ...
        @JavascriptInterface
        public String carpetaOneDrive() { return ""; }

        @JavascriptInterface
        public void eligeCarpetaOneDrive() {
            aviso("OneDrive no deja fijar una carpeta desde otra app. El parte se envia a OneDrive y alli eliges donde guardarlo.");
        }
        ...
        public void guardaOneDrive(String b64, String nombre) {
            ...
            runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    i.putExtra(Intent.EXTRA_SUBJECT, f.getName());
                    i.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                    i.setPackage(PAQUETE_ONEDRIVE);
                    try {
                        startActivity(i);
                    } catch (Exception sinOneDrive) {
                        Intent g = new Intent(Intent.ACTION_SEND);
                        g.setType("application/pdf");
                        g.putExtra(Intent.EXTRA_STREAM, PdfProvider.uriDe(f.getName()));
                        g.putExtra(Intent.EXTRA_SUBJECT, f.getName());
                        g.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                        try {
                            startActivity(Intent.createChooser(g, "Guardar el parte"));
                            aviso("OneDrive no esta instalado: elige donde guardarlo.");
                        } catch (Exception ex) {
